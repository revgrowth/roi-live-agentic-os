...
function Invoke-InWindowsPowerShell {
    ...
    if (-not [string]::IsNullOrEmpty($Title)) { $forwardArgs += @("-Title", $Title) }
    if (-not [string]::IsNullOrEmpty($Subtitle)) { $forwardArgs += @("-Subtitle", $Subtitle) }
    if (-not [string]::IsNullOrEmpty($Message)) { $forwardArgs += @("-Message", $Message) }
    if (-not [string]::IsNullOrEmpty($Attribution)) { $forwardArgs += @("-Attribution", $Attribution) }
    if (-not [string]::IsNullOrEmpty($Duration)) { $forwardArgs += @("-Duration", $Duration) }
    if (-not [string]::IsNullOrEmpty($Channel)) { $forwardArgs += @("-Channel", $Channel) }
    if (-not [string]::IsNullOrEmpty($Event)) { $forwardArgs += @("-Event", $Event) }
...
function Load-NotificationConfig {
    ...
    foreach ($path in @(
        "id", "displayName", "shortcutName", "assetVersion", "attribution", "defaultDuration"
    )) {
    ...
    $generatedHero = Require-Value $assets["generatedHero"] "assets.generatedHero"
    Require-Value $generatedHero["subtitle"] "assets.generatedHero.subtitle" | Out-Null

    $heroPaths = Require-Value $assets["heroPaths"] "assets.heroPaths"
    $variants = Require-Value $assets["variants"] "assets.variants"
    foreach ($variantName in @("success", "info", "warning", "error")) {
        ...
        foreach ($path in @("startColor", "endColor", "accentColor", "chipColor", "sound", "heroLabel")) {
    ...
    foreach ($channelName in $copyEvents.Keys) {
        ...
        foreach ($eventName in $copyEvents[$channelName]) {
            ...
            foreach ($path in @("title", "subtitle", "message", "variant", "duration")) {
                Require-Value $eventConfig[$path] "copy.$channelName.$eventName.$path" | Out-Null
            }
            Assert-AllowedValue -Value $eventConfig["variant"] -Allowed @("success", "info", "warning", "error") -Name "copy.$channelName.$eventName.variant"
...
function Get-NotificationPayload {
    ...
    $payloadDuration = if ([string]::IsNullOrWhiteSpace($Duration)) { $Config.app.defaultDuration } else { $Duration }
    $payloadTitle = if ([string]::IsNullOrWhiteSpace($Title)) { $Config.app.displayName } else { $Title }
    $payloadSubtitle = $Subtitle
    ...
    if (-not [string]::IsNullOrWhiteSpace($Channel) -or -not [string]::IsNullOrWhiteSpace($Event)) {
        ...
        $payloadVariant = $eventConfig.variant
        $payloadDuration = $eventConfig.duration
        $payloadTitle = Resolve-TemplateText -Template $eventConfig.title -Context $context
        if ([string]::IsNullOrWhiteSpace($payloadTitle)) {
            $payloadTitle = $Config.app.displayName
        }
        $payloadSubtitle = Resolve-TemplateText -Template $eventConfig.subtitle -Context $context
        $payloadMessage = Resolve-TemplateText -Template $eventConfig.message -Context $context
    }
...
function ConvertTo-Color { param([string]$Hex) return [System.Drawing.ColorTranslator]::FromHtml($Hex) }
function ConvertTo-XmlText { param([AllowEmptyString()][string]$Text) if ($null -eq $Text) { return "" } return [System.Security.SecurityElement]::Escape($Text) }
...
function New-RoundedRectPath {
    ...
    $path.AddArc($arc, 0, 90); $arc.X = $Rect.X
    $path.AddArc($arc, 90, 90); $path.CloseFigure()
    return $path
}

function New-NotificationLogo {
    param([string]$Path, [hashtable]$LogoConfig)
    ...
    $format.Dispose(); $textBrush.Dispose(); $font.Dispose(); $borderPen.Dispose()
    $overlayBrush.Dispose(); $backgroundBrush.Dispose(); $graphics.Dispose(); $bitmap.Dispose()
}

function New-NotificationHero {
    param([string]$Path, [hashtable]$AppConfig, [hashtable]$HeroConfig, [hashtable]$VariantConfig, [string]$VariantName)
    if (Test-Path -LiteralPath $Path) { return }
    $width = 364; $height = 180
    ...
    for ($offset = -120; $offset -lt 420; $offset += 24) { $graphics.DrawLine($linePen, $offset, $height, $offset + 100, 0) }
    $chipRect = New-Object System.Drawing.RectangleF(22, 22, 122, 28)
    $chipPath = New-RoundedRectPath -Rect $chipRect -Radius 14
    $chipBrush = New-Object System.Drawing.SolidBrush((ConvertTo-Color $VariantConfig.chipColor))
    ...
    $graphics.DrawString([string]$VariantConfig.heroLabel, $chipFont, $chipTextBrush, $chipRect, $chipFormat)
    ...
    $graphics.DrawString([string]$AppConfig.displayName, $headlineFont, $headlineBrush, 24, 64)
    $graphics.DrawString([string]$HeroConfig.subtitle, $subFont, $subBrush, 26, 102)
    ...
    $bitmap.Save($Path, [System.Drawing.Imaging.ImageFormat]::Png)
    $variantFont.Dispose(); $accentBrush.Dispose(); $subBrush.Dispose(); $subFont.Dispose()
    $headlineBrush.Dispose(); $headlineFont.Dispose(); $chipFormat.Dispose(); $chipTextBrush.Dispose()
    $chipFont.Dispose(); $chipBrush.Dispose(); $chipPath.Dispose(); $linePen.Dispose(); $glowBrush.Dispose()
    $backgroundBrush.Dispose(); $graphics.Dispose(); $bitmap.Dispose()
}

function Ensure-NotificationAssets {
    param([hashtable]$Config, [hashtable]$Payload)
    $assetRoot = Join-Path $env:LOCALAPPDATA "AgenticOS\notifications\$($Config.app.assetVersion)"
    New-Item -ItemType Directory -Force -Path $assetRoot | Out-Null
    $logoCustomPath = Resolve-ConfiguredPath $Config.assets.logoPath
    $heroCustomPath = Resolve-ConfiguredPath $Config.assets.heroPaths[$Payload.Variant]
    $logoPath = $logoCustomPath
    $heroPath = $heroCustomPath
    $logoSource = "custom"
    $heroSource = "custom"
    if ([string]::IsNullOrWhiteSpace($logoCustomPath) -or -not (Test-Path -LiteralPath $logoCustomPath)) {
    ...
    if ([string]::IsNullOrWhiteSpace($heroCustomPath) -or -not (Test-Path -LiteralPath $heroCustomPath)) {
        $heroPath = Join-Path $assetRoot "hero-$($Payload.Variant).png"
        $heroSource = "generated"
        New-NotificationHero -Path $heroPath -AppConfig $Config.app -HeroConfig $Config.assets.generatedHero -VariantConfig $Config.assets.variants[$Payload.Variant] -VariantName $Payload.Variant
    }
    return @{ AssetRoot = $assetRoot; LogoPath = $logoPath; LogoSource = $logoSource; HeroPath = $heroPath; HeroSource = $heroSource }
}

function Ensure-ShortcutInterop {
    if ("DesktopToastShortcut" -as [type]) { return }
...
function Ensure-StartMenuShortcut {
    param([string]$ShortcutPath, [hashtable]$Config, [string]$IconPath)
    Ensure-ShortcutInterop
    New-Item -ItemType Directory -Force -Path (Split-Path -Parent $ShortcutPath) | Out-Null
    [DesktopToastShortcut]::EnsureShortcut($ShortcutPath, $powershellExe, "-NoProfile", $projectDir, "$($Config.app.displayName) desktop notifications", $Config.app.id, $IconPath)
}

function Show-BalloonFallback {
    param([string]$FallbackTitle, [string]$FallbackMessage)
...
function Show-Toast {
    param([hashtable]$Config, [hashtable]$Payload, [hashtable]$Assets)
    $sound = ConvertTo-XmlText $Config.assets.variants[$Payload.Variant].sound
    $xmlContent = @"
<toast duration="$($Payload.Duration)">
  <visual>
    <binding template="ToastGeneric">
      <image placement="hero" src="$(ConvertTo-XmlText $Assets.HeroPath)" />
      <image placement="appLogoOverride" hint-crop="circle" src="$(ConvertTo-XmlText $Assets.LogoPath)" />
      <text>$(ConvertTo-XmlText $Payload.Title)</text>
      <text>$(ConvertTo-XmlText $Payload.Subtitle)</text>
      <text>$(ConvertTo-XmlText $Payload.Message)</text>
      <text placement="attribution">$(ConvertTo-XmlText $Payload.Attribution)</text>
    </binding>
  </visual>
  <audio src="$sound" />
</toast>
...
try {
    ...
    Ensure-StartMenuShortcut -ShortcutPath $shortcutPath -Config $config -IconPath $assets.LogoPath
    ...
    try {
        $fallbackTitle = if ([string]::IsNullOrWhiteSpace($payload.Subtitle)) { $payload.Title } else { "$($payload.Title) - $($payload.Subtitle)" }
        Show-BalloonFallback -FallbackTitle $fallbackTitle -FallbackMessage $payload.Message
        $result.ok = $true
