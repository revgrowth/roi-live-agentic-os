#!/usr/bin/env node
// Fetch the 4 target spoke URLs (for in-place audit-and-fix) plus the 2
// reference spokes (AC not cooling, AC blowing warm air) via Firecrawl
// /v2/scrape. Save markdown + metadata + heading outline + schema blocks +
// raw body HTML excerpt per spoke to audits/live-spoke-snapshots/.
//
// Patterned on _fetch-hubs.mjs (Phase 1 hub audit).
...
const targets = [
  // Reference patterns (already-good comprehensive spokes)
  { url: 'https://coastalcarolinahvac.com/blog/ac-not-cooling/', slug: 'ac-not-cooling', role: 'reference' },
  { url: 'https://coastalcarolinahvac.com/blog/ac-blowing-warm-air/', slug: 'ac-blowing-warm-air', role: 'reference' },
  // Target spokes (audit-and-fix in place)
  { url: 'https://coastalcarolinahvac.com/blog/ac-wont-turn-on/', slug: 'ac-wont-turn-on', role: 'target' },
  { url: 'https://coastalcarolinahvac.com/blog/ac-frozen-evaporator-coil/', slug: 'ac-frozen-evaporator-coil', role: 'target' },
  { url: 'https://coastalcarolinahvac.com/blog/ac-leaking-water-inside/', slug: 'ac-leaking-water-inside', role: 'target' },
  { url: 'https://coastalcarolinahvac.com/blog/ac-short-cycling/', slug: 'ac-short-cycling', role: 'target' },
];
...
for (const t of targets) {
  ...
  // Internal/external link summary
  const ccchvacLinks = links.filter(l => typeof l === 'string' && l.includes('coastalcarolinahvac.com'));
  out.push(`## Links to coastalcarolinahvac.com: ${ccchvacLinks.length}`);
  ccchvacLinks.slice(0, 60).forEach(l => out.push(`- ${l}`));
  out.push('');
