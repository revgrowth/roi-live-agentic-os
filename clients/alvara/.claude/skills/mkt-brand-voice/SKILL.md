...
If it doesn't exist → **Mode selection.** Use `AskUserQuestion` with:
- question: "How do you want to set up your brand voice?"
- options:
  - `import` — I have brand guidelines, import and map them across
  - `extract` — I have content samples, extract my voice from them
  - `build` — Start from scratch, ask me questions
  - `scrape` — Here's my URL, research it yourself
- default: `build`

**Fallback** (if AskUserQuestion unavailable): present the four options as a numbered list and accept a number or keyword in reply.
...
- options:
  - `quick` — 5-8 targeted questions, ~5 min
  - `playbook` — Full Agentic Academy interview, ~10-15 min (best if starting from zero)
- default: `quick`

**Fallback** (if AskUserQuestion unavailable): ask as plain text and accept "quick", "playbook", "1", or "2".
