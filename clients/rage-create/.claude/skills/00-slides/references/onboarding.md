...
**Optional:**
- `FIRECRAWL_API_KEY` in your `.env` — only needed if you want `mkt-design-tokens` to extract brand tokens from a URL automatically. Without it, you can paste tokens manually or run with neutral defaults.
- `brand_context/design-tokens.md` populated — pre-fills brand colors and typography. Run `/mkt-brand-voice` to populate, or paste tokens manually.
...
- **Brand identity:** `brand_context/design-tokens.md` (at project root or `agentic-os/`) — shared across all skills.
