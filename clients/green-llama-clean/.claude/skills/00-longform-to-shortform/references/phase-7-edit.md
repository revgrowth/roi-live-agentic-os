...
- `editing.subtitle_size` → --font-size (default: 102)
- `editing.subtitle_alignment` → --alignment (default: 2, bottom-center)
- `editing.subtitle_margin_v` → --margin-v (default: 350 for face/cursor-track, 0 for split-screen)
- `editing.hook_duration` → --hook-duration (default: 3.0)
- `editing.highlight_color` → --highlight-color (default: auto from design-tokens)
- `editing.subtitle_box_style` → --box-style (default: none)
...
If no thumbnail exists, the script produces a text-only card (no crash). The card is a 1080x1920 translucent PNG (`rgba` preserved) so video is visible through the edges.
...
For each clip, create a temporary HyperFrames project:

```bash
CLIP_HF_DIR="{run_dir}/clips/clip-N/hyperframes-project"
mkdir -p "$CLIP_HF_DIR"
cd "$CLIP_HF_DIR" && npx hyperframes init
```

Copy the reframed clip into the project directory so the HTML `<video>` element can reference it locally.
