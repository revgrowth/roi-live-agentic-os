...
2. **Never re-encode during clip extraction.** Use stream-copy (`-c copy`) for the initial extract. Re-encoding happens only during the reframe step.
...
4. **Groq has a 25MB file limit.** Extract audio as compressed MP3 before sending. If still over 25MB after re-encoding at lower bitrate, chunk and merge.
...
All settings come from `.claude/skills/00-longform-to-shortform/skill-pack/config/pipeline.config.yaml` in the workspace. Read it at the start and use its values throughout. If the file doesn't exist, use these defaults:
- format: `9x16`, layout: `split-screen`, whisper_model: `small`
- max_clips: `5`, min_clips: `2`, min_score: `65`, duration: `[45, 90]`, target_platform: `all`
...
| 4 | SELECT | 5-category scoring, clip_candidates.json. Pass `target_platform` from config to clip selection — when set, duration range adjusts to platform sweet spots per `vid-clip-selection` selection-framework.md | `references/phase-4-select.md` |
